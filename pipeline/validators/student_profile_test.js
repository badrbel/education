...
// 9. التحقق من ربط المستوى للرابعة متوسط (4AM) وحذف الساعات وشارة الترحيب
{
  console.log('\n--- 9. اختبار ربط مستوى 4AM وحذف بطاقة الساعات والترحيب المكرر ---');
  const env = createMockEnvironment();
  const saved4am = env.mockDom.window.StudentProfile.save('محمد', null, '4am');
  env.mockDom.window.StudentProfile.applyToUI(saved4am);

  assert(env.getEl('dashboard-level-name').textContent === 'السنة الرابعة متوسط', 'المستوى الحالي يظهر "السنة الرابعة متوسط" عند اختيار 4AM');
  assert(env.getEl('dashboard-branch-name').textContent === 'شهادة التعليم المتوسط (BEM)', 'الشعبة تظهر شهادة التعليم المتوسط (BEM) لـ 4AM');

  const htmlContent = fs.readFileSync(path.join(baseDir, 'index.html'), 'utf-8');
  assert(!htmlContent.includes('ساعات المذاكرة المنجزة'), 'بطاقة ساعات المذاكرة محذوفة تماماً من الواجهة');
  assert(!htmlContent.includes('مرحبًا بـك في mordix_ai'), 'عبارة شارة "مرحبًا بـك في mordix_ai" محذوفة تماماً من لوحة التحكم');
}
